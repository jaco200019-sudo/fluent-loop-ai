...
- AI 对话带场景约束和跑题保护，避免点咖啡场景回复旅行/会议内容
- AI 对话带输入质量判断，乱字母或不可理解英文会被纠正为可复制的自然表达
- 内容库扩展到 9 个对话场景：点咖啡、旅行、会议、面试、餐厅、酒店、购物、打车、看医生
- 新增餐厅/酒店/购物/打车/医生相关测试题、复习卡和高频词
- 新增今日小测页：选择题 + 情景造句，提交后显示对错、得分和下一步建议
- 生活课程库扩展到 15 个场景，新增租房、银行、快递退货、电话预约、紧急求助、社交寒暄等课程
- 新增本地学习数据数据库：记录任务完成、对话、小测、复习卡和能力测试，并支持导出 JSON
- 新增趣味学习机制：XP 经验值、等级进度、成就徽章、每日趣味挑战和完成庆祝反馈
- 新增朋友测试与发布准备中心：复制测试链接、分享应用、检查 PWA/AI/课程/数据状态，并可导出发布计划 JSON
- Today 今日计划
- AI 场景对话模拟
- 默认开启的中英对照/中文辅助开关
- 语音按钮真实状态和模拟识别
...
6. PWA 反馈稳定后，用 Capacitor 包装 iOS / Android，再准备应用商店素材、隐私政策和用户协议。
